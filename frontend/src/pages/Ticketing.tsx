...
const CATEGORIES: Category[] = ["Hardware", "Software", "Network", "Account", "Other"];
...
const statusConfig: Record<Status, { className: string; icon: React.ReactNode }> = {
  "Open": { className: "bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-300", icon: <CircleDot className="h-3 w-3" /> },
  "In Progress": { className: "bg-warning/10 text-warning", icon: <Clock className="h-3 w-3" /> },
  "Resolved": { className: "bg-success/10 text-success", icon: <CheckCircle2 className="h-3 w-3" /> },
  "Closed": { className: "bg-secondary text-muted-foreground", icon: <XCircle className="h-3 w-3" /> },
};

const statCards = [
  { label: "Open", status: "Open" as Status, icon: CircleDot, color: "text-blue-600" },
  { label: "In Progress", status: "In Progress" as Status, icon: Clock, color: "text-warning" },
  { label: "Resolved", status: "Resolved" as Status, icon: CheckCircle2, color: "text-success" },
  { label: "Closed", status: "Closed" as Status, icon: XCircle, color: "text-muted-foreground" },
];
...
function SubmitTicketModal({ open, onClose, onSuccess }: {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) { toast.error(data.message || "Failed to submit ticket"); return; }
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4 py-2">
          <div className="space-y-2">
            <Label>Title</Label>
            <Input placeholder="Brief summary of the issue" value={form.title} onChange={e => set("title", e.target.value)} />
          </div>
          <div className="space-y-2">
            <Label>Description</Label>
            <Textarea placeholder="Describe the issue in detail..." value={form.description} onChange={e => set("description", e.target.value)} className="resize-none h-20" />
          </div>
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${priorityVariant[ticket.priority]}`}>
              {ticket.priority}
            </span>
          </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {STATUSES.filter(s => s !== ticket.status).map(s => (
                  <button key={s} type="button"
                    onClick={() => { onStatusChange(ticket._id, s); onClose(); }}
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Submit and track your IT support requests</p>
        </div>
          ...
          {loading ? (
            ...
            <div className="divide-y divide-border">
              {tickets.map(ticket => {
                const sc = statusConfig[ticket.status];
                return (
                  <div key={ticket._id} onClick={() => onSelectTicket(ticket)}
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-base font-medium">
            ...
            {filterStatus !== "All" && <span className="text-muted-foreground font-normal"> · {filterStatus}</span>}
          </CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          {loading ? (
